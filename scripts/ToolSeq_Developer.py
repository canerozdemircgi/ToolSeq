...
class ToolSeq_Developer(QWidget):
	...
	def Event_Search(self):
		...
		if self.window.Check_CaseSensitive.isChecked():
			...
			filterList = [self.listPtr[i] for i in xrange(len(self.listPtr)) if filterKey in self.listPtr[i]]
		else:
			filterKey = self.window.Line_Search.text().strip().lower()
			filterList = [self.listPtr[i] for i in xrange(len(self.listPtr)) if filterKey in self.listPtr[i].lower()]

...
class ToolSeq_Unaverage(QWidget):
	...
	def __init__(self):
		...
		self.window = ToolSeq_Unaverage.qUiLoader.load(ToolSeq_Unaverage.userScriptDir + 'ToolSeq_Unaverage.ui', self)
	...
	def Action_Apply(self):
		...
		try:
			...
			objects = list(set([items[i].split('.')[0] for i in xrange(len(items))]))

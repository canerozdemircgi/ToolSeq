...
class ToolSeq_Randomizer(QWidget):
	...
	def __init__(self, state=None):
		...
		super(ToolSeq_Randomizer, self).__init__(self.windowMain)

		self.window = ToolSeq_Randomizer.qUiLoader.load(ToolSeq_Randomizer.userScriptDir + 'ToolSeq_Randomizer.ui', self)
		...
		self.window.Button_FunctReload.installEventFilter(self)
		...
		self.window.show()
		if state is None:
			self.window.setMinimumHeight(self.window.height() - 125)
			self.window.resize(self.window.width(), self.window.height() - 75)
		else:
		...
		if sourceObjectName == 'WindowMain':
			...
			if eventType == QEvent.MouseButtonRelease:
				if event.button() == Qt.LeftButton:
					self.Event_RandomsCount_3()
		elif sourceObjectName == 'Button_FunctReload':
			if eventType == QEvent.MouseButtonRelease:
				if event.button() == Qt.LeftButton:
					ToolSeq_Randomizer.Event_FunctReload()
	...
	@staticmethod
	def Event_FunctReload():
		reload(ToolSeq_Randoms)
	...
	def Calculate_Randoms(self, itemsLen, internalsLen=1):
		...
		if quantity == 'Single':
			...
			indexes = [i % len(datas) for i in xrange(randomsLen)]
		elif quantity == 'Seq 2>1':
			x = y = 0
			for i in xrange(internalsLen):
				for j in xrange(itemsLen):
					indexes[x] = y
					x += 1
				y = (y + 1) % len(datas)
		...
		functions = [compile('ToolSeq_Randoms.' + datas[i][0], '<string>', 'eval') for i in xrange(len(datas))]
		randoms = [eval(functions[indexes[i]], globals()) for i in xrange(randomsLen)]
		...
		if limit != 'Free':
			...
			if limit == 'Clamp':
				randoms = [max(limitMin, min(limitMax, randoms[i])) for i in xrange(randomsLen)]
			elif limit == 'Return':
		...
		if isInterval:
			mark += 1
			intervals = [float(datas[i][mark]) for i in xrange(len(datas))]
			for i in xrange(randomsLen):
		...
		if self.window.Check_FunctExclude.isChecked():
			mark += 1
			excludes = [[float(data) for data in datas[i][mark].strip().split(' ')] for i in xrange(len(datas))]
			i = 0
			while i < randomsLen:
	...
	def Action_Randomize(self):
		...
		try:
			...
			if self.window.Radio_ModeObject.isChecked():
				...
				for j in xrange(len(internals_)):
				...
				itemsSels = randomlib.sample(items_, random)

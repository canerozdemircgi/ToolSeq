...
class ToolSeq_Randomizer(QWidget):
	...
	def __init__(self, state=None):
		...
		with open(ToolSeq_Randomizer.userScriptDir + 'ToolSeq.qss') as fileStyleSheet:
	...
	def Calculate_Randoms(self, itemsLen, internalsLen=1):
		...
		datas = [s.strip().split('|') for s in self.window.Text_Randoms.toPlainText().strip().splitlines()]
		mark = 0
	...
	def Action_Randomize(self):
		...
		try:
			...
			if self.window.Radio_ModeObject.isChecked():
				items_ = [s.strip().split(' ') for s in itemsText.splitlines()]
				internals_ = [s.strip().split(' ') for s in internalsText.splitlines()]
				...
				internals = internalsText.splitlines()

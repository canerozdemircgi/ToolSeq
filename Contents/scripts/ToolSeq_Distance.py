...
class ToolSeq_Distance(QWidget):
	...
	def __init__(self):
		...
		with open(ToolSeq_Distance.userScriptDir + 'ToolSeq.qss') as fileStyleSheet:
	...
	def Event_CalculateDistance(self):
		distances = ToolSeq_Distance.Return_Distances(self.window.Text_Items.toPlainText().strip().splitlines())
		self.Activity_ShowValues(distances)

	def Event_CalculateBoundaries(self):
		boundaries = ToolSeq_Distance.Return_Boundaries(self.window.Text_Items.toPlainText().strip().splitlines())

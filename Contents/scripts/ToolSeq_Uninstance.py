...
class ToolSeq_Uninstance(QWidget):
	...
	def __init__(self):
		super(ToolSeq_Uninstance, self).__init__(ToolSeq_Uninstance.windowMain)

		self.window = ToolSeq_Uninstance.qUiLoader.load(ToolSeq_Uninstance.userScriptDir + 'ui/ToolSeq_Uninstance.ui', self)
		...
		with open(ToolSeq_Uninstance.userScriptDir + 'ToolSeq.qss') as fileStyleSheet:
	...
	def Action_UninstanceHierarchy(self):
		...
		try:
			maya_cmds.select(self.window.Text_Transforms.toPlainText().strip().splitlines(), hierarchy=True)

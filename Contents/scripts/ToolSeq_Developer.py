	...
	def Event_Search(self):
		...
		if self.window.Check_Regex.isChecked():
			if self.window.Check_CaseSensitive.isChecked():
				filterKey = re.compile(self.window.Line_Search.text().strip())
			else:
				filterKey = re.compile(self.window.Line_Search.text().strip(), re.IGNORECASE)
			filterList = [self.listPtr[i] for i in range(len(self.listPtr)) if filterKey.search(self.listPtr[i])]
		else:
			if self.window.Check_CaseSensitive.isChecked():

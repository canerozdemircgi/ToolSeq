...
class ToolSeq_Explode(QWidget):
	...
	def __init__(self):
		super(ToolSeq_Explode, self).__init__(ToolSeq_Explode.windowMain)

		self.window = ToolSeq_Explode.qUiLoader.load(ToolSeq_Explode.userScriptDir + 'ui/ToolSeq_Explode.ui', self)
		...
		with open(ToolSeq_Explode.userScriptDir + 'ToolSeq.qss') as fileStyleSheet:
	...
	def Action_Apply(self):
		...
		try:
			self.groupLow = maya_cmds.listRelatives(self.window.Text_GroupLow.toPlainText().strip().splitlines(), path=True, children=True, type='transform')
			self.groupHig = maya_cmds.listRelatives(self.window.Text_GroupHig.toPlainText().strip().splitlines(), path=True, children=True, type='transform')

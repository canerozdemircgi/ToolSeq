...
class ToolSeq_Freeze(QWidget):
	...
	def __init__(self):
		super(ToolSeq_Freeze, self).__init__(ToolSeq_Freeze.windowMain)

		self.window = ToolSeq_Freeze.qUiLoader.load(ToolSeq_Freeze.userScriptDir + 'ui/ToolSeq_Freeze.ui', self)
		...
		with open(ToolSeq_Freeze.userScriptDir + 'ToolSeq.qss') as fileStyleSheet:
		...
		self.window.Button_SelectUnFreezedScaleTransforms.installEventFilter(self)
	...
	def eventFilter(self, source, event):
		...
		elif sourceObjectName == 'Button_SelectUnFreezedScaleTransforms':
			if eventType == QEvent.MouseButtonRelease:
				if event.button() == Qt.LeftButton:
					self.Action_SelectUnFreezedScaleTransforms()
		return False
	...
	def Action_UnFreezeTranslate(self):
		...
		try:
			...
			pivotType = {}
			if pivotTypeText == 'Rotate Pivot':
				pivotType = {'rotatePivot': True}
			elif pivotTypeText == 'Scale Pivot':
	...
	def Action_UnFreezeScale(self):
		...
		try:
			transforms = self.window.Text_Transforms.toPlainText().strip().splitlines()
			isConsiderRot = self.window.Check_ConsiderRotation.isChecked()
			selection = []
			if isConsiderRot:
	...
	def Action_SelectUnFreezedScaleTransforms(self):
		maya_cmds.undoInfo(openChunk=True)
		try:
			transforms = self.window.Text_Transforms.toPlainText().strip().splitlines()
			selection = []
			for transform in transforms:
				scaleX = maya_cmds.getAttr(transform + '.scaleX')
				scaleY = maya_cmds.getAttr(transform + '.scaleY')
				scaleZ = maya_cmds.getAttr(transform + '.scaleZ')
				if scaleX not in (-1, 1) or scaleY not in (-1, 1) or scaleZ not in (-1, 1):

...
class ToolSeq_Randomizer_Apply(OpenMaya.MPxCommand):
	...
	def doIt(self, args):
		...
		self.randoms = PyObj_FromPtr(long(mArgParser.flagArgumentString(self.randomsFlag, 0), 0))
		self.items = PyObj_FromPtr(long(mArgParser.flagArgumentString(self.itemsFlag, 0), 0))
	...
	def redoIt(self):
		...
		try:
			...
			mPointsDict = {}
			mNormalsDict = {}
			for i in xrange(selections.length()):
				...
				if mDagPathName not in mPointsDict:
					...
					if objectType == 'mesh':
						...
						print >> stderr, 'Not supported object type: ' + objectType
				...
				for items in self.items:
					...
					for item in items:
						...
						if apiType == OpenMaya.MFn.kMeshVertComponent or apiType == OpenMaya.MFn.kCurveCVComponent or apiType == OpenMaya.MFn.kSurfaceCVComponent:
							...
							print >> stderr, 'Not supported component type: ' + mObj.apiTypeStr
